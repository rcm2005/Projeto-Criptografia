...
def codificar():
     ...
     escolha = int(input("deseja usar uma chave padrão ou enimga ?:\n 1 - Padrão \n 2 - Enigma \n "))
     
     if escolha == 1:
          palavra = input("Digite a mensagem (não use espaço ou acentos): ")
          chave = int(input('digite a chave do codigo: '))
          ...
          for o in range (len(letra)):
               ...
               for i in range(len(alfabeto)):
                    if letra[o] == alfabeto[i]:
                         posicaocyp = i + chave
                         posicaoori = i 
                         converted = alfabeto[posicaocyp]
                         palavracrypto.append(converted)
